...
function Socket(options) {
  ...
  } else if (typeof options == 'string' && toPort(options) === false) {
...
function connect(self, address, port, addressType) {
  ...
  if (addressType == 6) {
    var connectReq = self._handle.connect6(address, port);
  } else {
    var connectReq = self._handle.connect(address, port);
  }
...
Socket.prototype.connect = function(port /* [host], [cb] */) {
  ...
  var pipe = toPort(port) === false;
  var clazz = pipe ? Pipe : TCP;

  if (this.destroyed) {
    this._handle = new clazz();
    initSocketHandle(this);
  }
...
function listen(self, address, port, addressType) {
  ...
  self._handle = addressType ? new TCP() : new Pipe();
  self._handle.socket = self;
